...
void reshape(fixed16_t* input, fixed16_t* output);

// Perform dense layer
void perform_dense (float* input, float*output, const float*weight, const float* bias, int in_channel, int out_channel);
//void perform_dense (fixed16_t* input, fixed16_t*output, const fixed16_t*weight, const fixed16_t* bias, int in_channel, int out_channel);
